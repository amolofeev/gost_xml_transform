...
class GOSTXMLTransform:
    def __init__(self, element: etree.Element, parent: 'GOSTXMLTransform' = None):
        self.parent = parent
        self.generator = iter(range(1, 100000)) if parent is None else None
        self.children: List['GOSTXMLTransform'] = []
    ...
    def _parse_attrib(self) -> None:
        ...
        self.attrib = [
            [*attr[:3], self._get_ns(attr[0]) if attr[-1] else None]
            for attr in attrib_list
        ]

    def _get_next_id(self):
        """Get next sequence value. Sequence must be defined in the root element only"""
        if not self.generator:
            return self.parent._get_next_id()
    ...
    def _parse_nsmap(self) -> None:
        ...
        WARNING: sort algorithm could be wrong.
        """
        self.prefix = self._get_ns(self.tag.namespace)
    ...
    def to_bytes(self, out=None, is_root=True) -> Union[bytes, None]:
        """
        ...
        out.write(f'<{self.prefix + ":" if self.prefix else ""}{self.tag.localname}'.encode('utf-8'))
        out.write(self._nsmap_to_string().encode('utf-8'))
        out.write(self._attrib_to_string().encode('utf-8'))
        out.write(b'>')
        out.write(self.text.encode('utf-8'))
        for child in self.children:
            child.to_bytes(out, False)
        out.write(f'</{self.prefix + ":" if self.prefix else ""}{self.tag.localname}>'.encode('utf-8'))
    ...
    def from_string(cls, xml_string: str, encoding: str = 'utf-8') -> 'GOSTXMLTransform':
        ...
        return GOSTXMLTransform.from_bytes(
            xml_string.encode(encoding)
        )
